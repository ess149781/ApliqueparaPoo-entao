		...
		do {
			...
			if (n==1) {
				...
				if (f==1) {
					...
					formas.add(new Quadrado(p1,0,0));
				} else if (f==2) {
					System.out.print("Informe a base: ");
					p1 = Double.parseDouble(in.readLine());													
					System.out.print("\nInforme a altura: ");
					p2 = Double.parseDouble(in.readLine());													
					formas.add(new Retangulo(p1,p2,0,0));
				} else if (f==3) {
					System.out.print("Informe o raio: ");
					p1 = Double.parseDouble(in.readLine());													
					formas.add(new Circulo(p1,0,0));
				}				
			} else if (n==2) {
				double soma = 0, area=0;
				for (int i=0; i<formas.size();++i) {
					area = formas.get(i).calculaArea();
				//	System.out.println("Area do "+ ((Forma) formas.get(i)).getNome() + ": " + area);
					System.out.println("Area do "+ formas.get(i).getNome() + ": " + area);
					soma += area;
				}
